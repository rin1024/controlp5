 ...
 *
 * <p>A Controller can notify the main program in 2 different ways:
 *
 * <ul>
 *   <li>(1) add method controlEvent(ControlEvent theEvent) to your sketch. ControlP5 will
 *       automatically detect this method and will used it to forward any controlEvent triggered by
 *       a controller - you can disable forwarding by using setBroadcast(false) {@link
 *       controlP5.Controller#setBroadcast(boolean)}
 *   <li>(2) each controller requires a unique name when being create. In case an existing name is
 *       used for a newly created Controller, the existing one will be overwritten. each unique name
 *       can be used to automatically link a controller to either a method or a field within your
 *       program.
 * </ul>
 *
 * @see controlP5.Bang
...
public abstract class Controller<T>
        ...
        if (getControllerPlugList().get(0).getValue() == null) {
          ...
          if (getControllerPlugList().get(0).getValue() instanceof Boolean) {
            final boolean myBoolean =
                new Boolean(getControllerPlugList().get(0).getValue().toString()).booleanValue();
            myInitValue = (myBoolean == true) ? 1f : 0f;

          } else if (getControllerPlugList().get(0).getValue() instanceof Float) {
            myInitValue =
                (new Float(getControllerPlugList().get(0).getValue().toString())).floatValue();

          } else if (getControllerPlugList().get(0).getValue() instanceof Integer) {
            myInitValue =
                (new Integer(getControllerPlugList().get(0).getValue().toString())).intValue();
  ...
  /**
   * Use setBroadcast to enable and disable the broadcasting of changes in a controller's value. By
   * default any value changes are forwarded to function controlEvent inside your program. use
  ...
  /**
   * check if broadcasting is enabled or disabled for a controller. Every event relevant for a value
   * change will be broadcasted to any of the value-listeners. By default broadcasting for a
      ...
      if (isMousePressed && cp5.isAltDown() && isMoveable) {
        if (!cp5.isMoveable()) {
          ...
          if (cp5.isShiftDown()) {
            set(
                position,
                ((int) (x(positionBuffer)) / 10) * 10,
                ((int) (y(positionBuffer)) / 10) * 10);
          } else {
    ...
    return new Pointer() {
      ...
      public String toString() {
        return String.format(
            "{ x: %d y: %d px: %d py: %d dx: %d dy: %d}", x(), y(), px(), py(), dx(), dy());
      }
    };
  }
   ...
   * function will take care of default matrix operations and will call the display function of the
   * current ControllerView object active for this particular controller.
   *
   * @exclude
   ...
   * can't find a tab with given name, controlP5 will create this tab and add it to the main window.
   *
  ...
  public final T moveTo(
      final ControllerGroup<?> theGroup, final Tab theTab, ControlWindow theControlWindow) {
    for (Controller<?> c : getSubelements()) {
  ...
  public final T setParent(final ControllerInterface<?> theParent) {
    ...
    if (theParent != null) {
      ...
      set(
          absolutePosition,
          x(absolutePosition) + x(_myParent.getPosition()),
          y(absolutePosition) + y(_myParent.getPosition()));
  ...
  protected void addControllerPlug(final ControllerPlug thePlug) {
    if (checkControllerPlug(thePlug)) return;
   ...
   * visually update the controller. the updating status can be set with setUpdate(true/false) and
   * checked with isUpdate().
   *
  ...
  /**
   * set or change the value of the value label of a controller. (this is cheating, but maybe useful
   * for some cases.)
   *
   * @param theLabel
   * @return Controller
   ...
   * This can be handy for buttons, toggles, bangs, for more complex controllers such as sliders,
   * range, dropdownlist this is not advisable.
   *
  ...
  protected String adjustValue(final float theValue, final int theFloatPrecision) {
    ...
    if (myIndex > 0) {
      ...
      myLabelValue =
          myLabelValue.substring(
              0, (int) Math.min(myLabelValue.length(), myIndex + myFloatNumberLength));
  ...
  /**
   * convenience method to fill a float array in favor of theArray[0] = 1.2; etc. takes a float
   * array and fills it (starting from index 0) with arguments starting from index 1.
   */
