...
public class ControlP5Base extends ControlP5Legacy implements ControlP5Constants {
  ...
  protected Map<Object, ArrayList<ControllerInterface<?>>> _myObjectToControllerMap =
      new HashMap<Object, ArrayList<ControllerInterface<?>>>();
  protected Map<String, FieldChangedListener> _myFieldChangedListenerMap =
      new HashMap<String, FieldChangedListener>();
   ...
   * from 0-100.
   *
   * <p>By default it will be added to the default tab of the main window. Sliders can be arranged
   * vertically and horizontally depending on their width and height. The look of a sliders control
   * can either be a bar or a handle. you can add tickmarks to a slider or use the default
  ...
  public Range addRange(final Object theObject, final String theName) {
    return addRange(
        theObject,
        theObject != null ? theObject.toString() : "",
        theName,
        0,
        100,
        0,
   ...
   * control areas for a knob, an area closer to the edge allows 'click-and-adjust' control, a click
   * and drag action at the inside allows to gradually change the value of a know when dragged. A
   * knob can be controller by mouse click, drag or mouse-wheel.
   */
  ...
  public Matrix addMatrix(final Object theObject, final String theName) {
    return addMatrix(
        theObject, theObject != null ? theObject.toString() : "", theName, 10, 10, 0, 0, 100, 100);
  }
  ...
  public Textfield addTextfield(final Object theObject, final String theIndex) {
    return addTextfield(
        theObject, theObject != null ? theObject.toString() : "", theIndex, 0, 0, 200, 20);
  }
  ...
  /**
   * the ScrollableList replaces the DropwdownList and ListBox, the type for a ScrollableList can be
   * set with setType(ControlP5.DROPDOWN | ControlP5.LIST).
   */
  ...
  public ColorWheel addColorWheel(final Object theObject, final String theName) {
    return addColorWheel(
        theObject, theObject != null ? theObject.toString() : "", theName, 0, 0, 200);
  }
  ...
  public ColorPicker addColorPicker(final Object theObject, final String theName) {
    return addColorPicker(
        theObject, theObject != null ? theObject.toString() : "", theName, 0, 0, 255, 10);
  }
  ...
  public FrameRate addFrameRate() {
    FrameRate myController =
        new FrameRate(cp5, (Tab) cp5.controlWindow.getTabs().get(1), "-", 0, 4);
   ...
   *
   * <p>Uses a forward slash delimited address, for example:
   *
   * <p>lets say the theAddressSpace parameter is set to "hello", and the Object (second parameter)
   * contains an annotated field "x", addControllersFor("hello", o); will add a controller for field
   * x with address /hello/x This address can be used with getController("/hello/x") to access the
   * controller of that particular Object and field.
   */
   ...
   *
   * <p>printed Format: returnType methodName(parameter type)
   */
  public static void printPublicMethodsFor(Class<?> theClass, String... thePattern) {
    ...
    str += "*\n";
    str +=
        "* find a list of public methods available for the "
    ...
    str +=
        "You can find further details about class "
            + theClass.getSimpleName()
            + " in the javadoc.\n\n";
    str += "Format:\n";
  ...
  public static Set<String> getPublicMethodsFor(
      Class<?> theClass, boolean isSuperclass, String... thePattern) {
            ...
            s.add(
                c.getCanonicalName()
                    ...
                    + method
                        .getReturnType()
                        .getSimpleName()
                        .replace("Object", theClass.getSimpleName())
                    + " "
                    + method.getName()
                    + "("
                    + params
