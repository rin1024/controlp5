...
public final class ControlWindow {
  ...
  /**
   * Sets the position of the tab bar which is set to 0,0 by default. to move the tabs to y-position
   * 100, use cp5.getWindow().setPositionOfTabs(0,100);
   */
  ...
  private void handleMouseWheelMoved() {
    if (mouseWheelMoved != 0) {
      List<ControllerInterface<?>> mouselist =
          new CopyOnWriteArrayList<ControllerInterface<?>>(mouseoverlist);
      for (ControllerInterface<?> c : mouselist) {
        if (c.isVisible()) {
          if (c instanceof Controller) {
            ((Controller) c).onScroll(mouseWheelMoved);
            cp5.getControlBroadcaster()
                .invokeAction(new CallbackEvent((Controller) c, ControlP5.ACTION_WHEEL));
  ...
  /**
   * by default the background of a controlWindow is filled with a background color every frame. to
   * enable or disable the background from drawing, use setDrawBackgorund(true/false).
   *
   * @param theFlag
   * @return ControlWindow
   */
   ...
   * pressed and released state. The pointer can be accessed by its getter method {@link
   * ControlWindow#getPointer()}. Then use {@link controlP5.ControlWindow#set(int, int)} to alter
   * its position or invoke { {@link controlP5.ControlWindow#pressed()} or {@link
   * controlP5.ControlWindow#released()} to change its state. To disable the mouse and enable the
   * Pointer use {@link controlP5.ControlWindow#enable()} and {@link
   * controlP5.ControlWindow#disable()} to default back to the mouse as input parameter.
   */
  ...
  public class Pointer {
    ...
    // TODO mousePressed/mouseReleased are handled wrongly, released is called when moved, for now
    // do not use, instead use set(x,y), pressed(), released()
