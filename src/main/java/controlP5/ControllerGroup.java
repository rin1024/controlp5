...
public abstract class ControllerGroup<T>
  ...
  void setParent(ControllerGroup<?> theParent) {
    ...
    set(
        absolutePosition,
        x(absolutePosition) + x(_myParent.absolutePosition),
        y(absolutePosition) + y(_myParent.absolutePosition));
  ...
  public T updateAbsolutePosition() {
    ...
    set(
        absolutePosition,
        x(absolutePosition) + x(_myParent.getAbsolutePosition()),
        y(absolutePosition) + y(_myParent.getAbsolutePosition()));
    for (int i = 0; i < controllers.size(); i++) {
  ...
  /**
   * Adds a canvas to a controllerGroup such as a tab or group. Use processing's draw methods to add
   * visual content.
   */
