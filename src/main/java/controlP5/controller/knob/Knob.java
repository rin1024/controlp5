package controlP5.controller.knob;
...
public class Knob extends Controller<Knob> {
  ...
  /**
   * Sets the drag direction, when controlling a knob, parameter is either Controller.HORIZONTAL or
   * Controller.VERTICAL.
   *
  ...
  /**
   * set the display style of a knob. takes parameters Knob.LINE, Knob.ELLIPSE or Knob.ARC. default
   * style is Knob.LINE
   *
    ...
    public void display(PGraphics theGraphics, Knob theController) {
      ...
      if (getViewStyle() == Controller.LINE) {
        ...
        theGraphics.arc(
            ...
            getRadius() * 1.8f,
            getStartAngle(),
            getAngle() + ((getStartAngle() == getAngle()) ? 0.06f : 0f));
        theGraphics.fill(
            theGraphics.red(getColor().getBackground()),
            theGraphics.green(getColor().getBackground()),
            theGraphics.blue(getColor().getBackground()),
            255);
