 ...
 *
 * <p>a radioButton has 2 sets of values. radioButton.getValue() returns the value of the active
...
public class RadioButton extends ControlGroup<RadioButton> {
  ...
  /**
   * Items of a radioButton or a checkBox are organized in columns and rows. SetItemsPerRow sets the
   * limit of items per row. items exceeding the limit will be pushed to the next row.
   *
   * @param theValue
   */
  ...
  /**
   * In order to always have 1 item selected, use setNoneSelectedAllowed(false), by default this is
   * true. setNoneSelectedAllowed does not apply when in multipleChoice mode.
   *
   * @param theValue
   */
