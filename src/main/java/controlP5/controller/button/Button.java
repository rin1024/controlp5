 ...
 * fields inside your program/sketch. for a full documentation of this controller see the {@link
 * Controller} class.
 *
...
public class Button extends Controller<Button> {
  ...
  /**
   * If the button acts as a switch, setOn will turn on the switch. Use {@link
   * controlP5.Button#setSwitch(boolean) setSwitch} to turn a Button into a Switch.
   *
   * @return Button
   */
  public Button setOn() {
  ...
  /**
   * If the button acts as a switch, setOff will turn off the switch. Use {@link
   * controlP5.Button#setSwitch(boolean) setSwitch} to turn a Button into a Switch.
   *
   * @return Button
   */
  ...
  private class ButtonImageView implements ControllerView<Button> {
    ...
    public void display(PGraphics theGraphics, Button theController) {
      if (isOn && isSwitch) {
        theGraphics.image(
            (availableImages[HIGHLIGHT] == true) ? images[HIGHLIGHT] : images[DEFAULT], 0, 0);
        return;
      }
      if (isInside()) {
        if (isPressed) {
          theGraphics.image(
              (availableImages[ACTIVE] == true) ? images[ACTIVE] : images[DEFAULT], 0, 0);
        } else {
