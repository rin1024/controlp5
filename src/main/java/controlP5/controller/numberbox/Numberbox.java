 ...
 *
 * <p>Why do I get -1000000 as initial value when creating a numberbox without a default value? the
 ...
 *
 * <p>Use setMultiplier(float) to change the sensitivity of values increasing/decreasing, by default
 * the multiplier is 1.
 *
...
public class Numberbox extends Controller<Numberbox> {
   ...
   * of a multi-touch track pad. The smaller the value (closer to 0) the higher the sensitivity.
   *
   * @param theValue
   * @return Numberbox
   */
  ...
  public Numberbox scrolled(int theRotationValue) {
    ...
    f +=
        (_myMultiplier == 1)
            ? (theRotationValue * scrollSensitivity)
            : theRotationValue * _myMultiplier;
    setValue(f);
    return this;
  }

  /**
   * set the direction for changing the numberbox value when dragging the mouse. by default this is
   * up/down (VERTICAL), use setDirection(Controller.HORIZONTAL) to change to left/right or back
   * with setDirection(Controller.VERTICAL).
   *
   * @param theValue
   */
