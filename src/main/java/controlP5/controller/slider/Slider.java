...
public class Slider extends Controller<Slider> {
  ...
  public Slider setSliderMode(int theMode) {
    ...
    if (_mySliderMode == FLEXIBLE) {
      _myHandleSize =
          (_myDefaultHandleSize >= getHeight() / 2)
              ? _myDefaultHandleSize / 2
              : _myDefaultHandleSize;
    } else {
      _myHandleSize = 0;
    }
    _myView.updateUnit();
  ...
  /**
   * sets the sensitivity for the scroll behavior when using the mouse wheel or the scroll function
   * of a multi-touch track pad. The smaller the value (closer to 0) the higher the sensitivity. by
   * default this value is set to 0.1
   *
   * @param theValue
   * @return Slider
   */
  ...
  /**
   * changes the value of the slider when hovering and using the mouse wheel or the scroll function
  ...
  private class SliderViewV extends SliderView {
    ...
    void setSnapValue() {
      float n =
          PApplet.round(PApplet.map(_myValuePosition, 0, getHeight(), 0, _myTickMarks.size() - 1));
    ...
    public void display(PGraphics theGraphics, Slider theController) {
      ...
      theGraphics.fill(isInside() ? getColor().getActive() : getColor().getForeground());
      if (getSliderMode() == FIX) {
        ...
        if (isShowTickMarks) {
          theGraphics.triangle(
              getWidth(),
              getHeight() - getValuePosition(),
              getWidth(),
              getHeight() - getValuePosition() - getHandleSize(),
              0,
              getHeight() - getValuePosition() - getHandleSize() / 2);
        } else {
          theGraphics.rect(
              0, getHeight() - getValuePosition() - getHandleSize(), getWidth(), getHandleSize());
        }
      }
      ...
      if (isLabelVisible) {
        ...
        theGraphics.translate(
            0,
            (int)
                PApplet.map(_myValue, _myMax, _myMin, 0, getHeight() - _myValueLabel.getHeight()));
      ...
      if (isShowTickMarks) {
        ...
        float x =
            (getHeight() - ((getSliderMode() == FIX) ? 0 : getHandleSize()))
                / (getTickMarks().size() - 1);
  ...
  private class SliderViewH extends SliderView {
    ...
    void setSnapValue() {
      float n =
          PApplet.round(PApplet.map(_myValuePosition, 0, getWidth(), 0, _myTickMarks.size() - 1));
      _myValue = PApplet.map(n, 0, _myTickMarks.size() - 1, _myMin, _myMax);
    }
    ...
    public void display(PGraphics theGraphics, Slider theController) {
      ...
      if (getSliderMode() == FIX) {
        ...
        if (isShowTickMarks) {
          theGraphics.triangle(
              getValuePosition(),
              0,
              getValuePosition() + getHandleSize(),
              0,
              getValuePosition() + _myHandleSize / 2,
              getHeight());
        } else {
      ...
      if (isShowTickMarks) {
        ...
        theGraphics.noStroke();
        float x =
            (getWidth() - ((getSliderMode() == FIX) ? 0 : getHandleSize()))
                / (getTickMarks().size() - 1);
        for (TickMark tm : getTickMarks()) {
          tm.draw(theGraphics, getDirection());
