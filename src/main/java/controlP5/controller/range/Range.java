...
public class Range extends Controller<Range> {
  ...
  public Range(
    ...
    minHandle =
        (int) PApplet.map(theDefaultMinValue, _myMin, _myMax, handleSize, getWidth() - handleSize);
    maxHandle =
        (int) PApplet.map(theDefaultMaxValue, _myMin, _myMax, handleSize, getWidth() - handleSize);
    mr = maxHandle - minHandle;

    _myCaptionLabel =
        new Label(cp5, theName).setColor(color.getCaptionLabel()).align(RIGHT_OUTSIDE, CENTER);
    _myValueLabel =
        new Label(cp5, "" + adjustValue(_myMin))
            .setColor(color.getValueLabel())
            .set("" + adjustValue(theDefaultMinValue))
            .align(LEFT, CENTER);
    _myHighValueLabel =
        new Label(cp5, adjustValue(_myMax))
            .setColor(color.getValueLabel())
            .set("" + adjustValue(theDefaultMaxValue))
            .align(RIGHT, CENTER);
  ...
  public Range updateInternalEvents(PApplet theApplet) {
    if (isVisible) {
      int c = _myControlWindow.getPointer().getX() - _myControlWindow.getPointer().getPreviousX();
      if (c == 0) {
        return this;
      }
      if (isMousePressed && !cp5.isAltDown()) {
        switch (mode) {
          ...
          case (CENTER):
            minHandle =
                PApplet.max(handleSize, PApplet.min(getWidth() - mr - handleSize, minHandle + c));
            maxHandle =
                PApplet.max(minHandle, PApplet.min(getWidth() - handleSize, minHandle + mr));
            break;
        }
        update();
      }
  ...
  public void mousePressed() {
    ...
    if (_myControlWindow.getPointer().getY() < posY
        || _myControlWindow.getPointer().getY() > posY + getHeight()) {
      mode = -1;
    ...
    if (_myControlWindow.getPointer().getX() >= x0 - handleSize
        && _myControlWindow.getPointer().getX() < x0) {
      mode = LEFT;
      isMinHandle = true;
    } else if (_myControlWindow.getPointer().getX() >= x1
        && _myControlWindow.getPointer().getX() < x1 + handleSize) {
      mode = RIGHT;
      isMaxHandle = true;
    } else if (_myControlWindow.getPointer().getX() > x0
  ...
  private Range setLowValue(float theValue, boolean isUpdate) {
    ...
    minHandle =
        (int) PApplet.map(_myArrayValue[0], _myMin, _myMax, handleSize, getWidth() - handleSize);
  ...
  private Range setHighValue(float theValue, boolean isUpdate) {
    ...
    maxHandle =
        (int) PApplet.map(_myArrayValue[1], _myMin, _myMax, handleSize, getWidth() - handleSize);
    return (isUpdate) ? update() : this;
  }
  ...
  class RangeView implements ControllerView<Range> {
    ...
    public void display(PGraphics theGraphics, Range theController) {
      ...
      if (isShowTickMarks) {
        ...
        theGraphics.fill(
            (isMaxHandle || high == RIGHT) ? color.getActive() : color.getForeground());
  ...
  @Override
  public Range setFont(PFont thePFont) {
    _myHighValueLabel.setFont(thePFont);
    return super.setFont(thePFont);
  }

  @Override
  public Range setFont(ControlFont theFont) {
