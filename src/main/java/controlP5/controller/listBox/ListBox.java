package controlP5.controller.listbox;

/**
 * controlP5 is a processing gui library.
 *
 * <p>2006-2012 by Andreas Schlegel
 *
...
import controlP5.controller.slider.Slider;
...
public class ListBox extends ControlGroup<ListBox> implements ControlListener {
  ...
  public ListBox setItemHeight(int theHeight) {
    ...
    for (int i = 0; i < buttons.size(); i++) {
      ...
      buttons.get(i).y(new float[] {(float) ((theHeight + spacing) * i)});
    }
  ...
  public ListBox setHeight(int theHeight) {
    ...
    _myBackgroundHeight =
        (_myOriginalBackgroundHeight / (_myItemHeight + spacing)) * (_myItemHeight + spacing);
  ...
  public ListBox updateInternalEvents(PApplet theApplet) {
    boolean xx =
        cp5.getWindow().getPointer().getX() > getAbsolutePosition()[0]
            && cp5.getWindow().getPointer().getX() < getAbsolutePosition()[0] + _myWidth;
    // there is a 1px gap between bar and controllers, so -1 the top-y-position
    boolean yy =
        cp5.getWindow().getPointer().getY() > getAbsolutePosition()[1] - 1
            && cp5.getWindow().getPointer().getY() < getAbsolutePosition()[1] + _myBackgroundHeight;
    isInsideGroup = isOpen() ? xx && yy : false;
    if ((isBarVisible ? isInside : false) || isInsideGroup) {
      // updateInternalEvents(cp5.getApp());
    }
    return this;
  }
  ...
  public ListBox setWidth(int theWidth) {
    _myWidth = theWidth;
    updateButtonWidth();
    _myScrollbar.x(new float[] {(float) (_myWidth - _myScrollbarWidth)});
    return this;
  }
  ...
  protected ListBox addListButton(int theNum) {
    for (int i = 0; (i < theNum) && (buttons.size() < maxButtons); i++) {
      ...
      Button b =
          new Button(
              cp5,
              (ControllerGroup<?>) this,
              _myName + "Button" + index,
              index,
              0,
              index * (_myItemHeight + spacing),
              _myWidth,
              _myItemHeight);
      b.setMoveable(false);
      add(b);
      ...
      // b.addListener(this); // TODO: fix me, go to loop....
      buttons.add(b);
    }
    updateScroll();
    return this;
  ...
  public ListBox removeItem(String theItemName) {
    try {
      ...
      L.info("ScrollList.removeItem exception:" + e);
    }
    return this;
  }
  ...
  public void controlEvent(ControlEvent theEvent) {
    if (theEvent.getController() instanceof Button) {
      try {
        setValue(theEvent.getController().getValue());
  ...
  public void keyEvent(final KeyEvent theEvent) {
    ...
    float x = getAbsolutePosition()[0];
    float y = getAbsolutePosition()[1];
    boolean b =
        (getWindow().getPointer().getX() > x
            && getWindow().getPointer().getX() < (x + _myWidth)
            && getWindow().getPointer().getY() > (y - getBarHeight())
            && getWindow().getPointer().getY() < y + _myOriginalBackgroundHeight);
    if (b && isOpen()) {
  ...
  public String[][] getListBoxItems() {
    ...
    for (int i = 0; i < l.length; i++) {
      l[i] =
          new String[] {items.get(i).name, items.get(i).text, Integer.toString(items.get(i).value)};
    }
    return l;
  }
