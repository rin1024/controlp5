    ...
    public void display(PGraphics theGraphics, Chart theController) {
      ...
      while (it.hasNext()) {
        ...
        for (int i = 0; i < s; i++) {
          ...
          float ww = ((getWidth() / s));
          float hh =
              PApplet.map(getDataSet(index).get(i).getValue(), _myMin, _myMax, 0, getHeight());
          theGraphics.rect(
              o + i * ww,
              getHeight(),
              (ww / getDataSet().size()),
    ...
    public void display(PGraphics theGraphics, Chart theController) {
      ...
      while (it.hasNext()) {
        ...
        for (int i = 0; i < getDataSet(index).size(); i++) {
          float hh =
              PApplet.map(getDataSet(index).get(i).getValue(), _myMin, _myMax, getHeight(), 0);
