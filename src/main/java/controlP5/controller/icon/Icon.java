...
public class Icon extends Controller<Icon> {
  ...
  /** A Icon can be activated by a mouse PRESSED or mouse RELEASE. Default value is RELEASE. */
  ...
  /**
   * If the Icon acts as a switch, setOn will turn on the switch. Use {@link
   * controlP5.Icon#setSwitch(boolean) setSwitch} to turn a Icon into a Switch.
   */
  public Icon setOn() {
    if (isSwitch) {
      isOn = false;
  ...
  /**
   * If the Icon acts as a switch, setOff will turn off the switch. Use {@link
   * controlP5.Icon#setSwitch(boolean) setSwitch} to turn a Icon into a Switch.
   */
  public Icon setOff() {
    if (isSwitch) {
  ...
  /**
   * Returns true or false and indicates the switch state of the Icon. {@link setSwitch(boolean)
  ...
  private class IconView implements ControllerView<Icon> {
    ...
    public void display(PGraphics theGraphics, Icon theController) {
      ...
      _myCaptionLabel.setColor(
          isOn && isSwitch || isPressed && !isSwitch ? color.getActive() : color.getForeground());
  ...
  private class IconImageView implements ControllerView<Icon> {
    ...
    public void display(PGraphics theGraphics, Icon theController) {
      ...
      if (isOn && isSwitch) {
        theGraphics.image(
            (availableImages[HIGHLIGHT] == true) ? images[HIGHLIGHT] : images[DEFAULT],
            -w_half,
            -h_half);
        return;
      }
      if (getIsInside()) {
        if (isPressed) {
          ...
          theGraphics.image(
              (availableImages[OVER] == true) ? images[OVER] : images[DEFAULT], -w_half, -h_half);
        }
      } else {
