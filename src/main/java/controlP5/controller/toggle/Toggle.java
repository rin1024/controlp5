...
public class Toggle extends Controller<Toggle> {
  ...
  /**
   * by default a toggle returns 0 (for off) and 1 (for on). the internal value variable can be used
   * to store an additional value for a toggle event.
   *
  ...
  @Override
  public Toggle setImages(PImage... theImages) {
  ...
  class ToggleSwitchView implements ControllerView<Toggle> {
    ...
    public void display(PGraphics theGraphics, Toggle theController) {
      ...
      if (isOn) {
        ...
        theGraphics.rect(
            (getWidth() % 2 == 0 ? 0 : 1) + getWidth() / 2, 0, getWidth() / 2, getHeight());
      }
