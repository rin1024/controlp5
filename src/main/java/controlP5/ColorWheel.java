...
public class ColorWheel extends Controller<ColorWheel> {
  ...
  private void setColorResources() {
    ...
    buffer.ellipse(
        buffer.width / 2, buffer.height / 2, buffer.width * scalar + 4, buffer.width * scalar + 4);
    buffer.fill(255);
    buffer.ellipse(
        buffer.width / 2, buffer.height / 2, (inner_radius + 1) * 2, (inner_radius + 1) * 2);
  ...
  public ColorWheel setRGB(int theColor) {
    ...
    // https://github.com/sojamo/controlp5/issues/21
    _myColorValue = HSLtoRGB(hsl);
    setValue(_myColorValue);
    return this;
  }
  ...
  private class ColorWheelView implements ControllerView<ColorWheel> {
    ...
    public void display(PGraphics theGraphics, ColorWheel theController) {
      ...
      if (isInfo) {
        _myInfoLabel.setText(
            String.format(
                "RGB %d %d %d\nALPHA %d\nHSL %d %.2f %.2f ",
                r(), g(), b(), a(), (int) (hsl[0] * 360), hsl[1], hsl[2]));
